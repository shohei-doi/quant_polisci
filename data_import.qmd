...
RやPythonではパッケージにサンプルデータが付属していることが多いです。

例えば、Rの場合は`data()`を実行することで、読み込んでるパッケージに付属するデータを確認できます。

```{r filename="R"}
data()
```

呼び出したいデータセットの名前を入れて`data(...)`と実行するとデータを呼び出すことができます。
...
Pythonの場合、例えば`seaborn`では`sns.get_dataset_names()`でデータを確認できます。
...
`sns.load_dataset(...)`でデータを呼び出します。

```{python filename="Python"}
sns.load_dataset("titanic")
```

また、データ分析で用いる`statsmodels`や`scikit-learn`などのパッケージでもデータを呼び出すことができます。
