...
```{r}
library(tidyverse)
data <- read_csv("data/triangle.csv")
```
...
head(select(data, demauta, demautb, demautm))
```
...
```{r}
data$mindep <- ifelse(data$dependa < data$demautb, data$dependa, data$dependb)
head(select(data, dependa, dependb, smldep, mindep))
```
